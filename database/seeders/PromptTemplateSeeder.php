...
class PromptTemplateSeeder extends Seeder
{
    public function run(): void
    {
        $templates = [
            ...
            [
                'key' => 'HOMEPAGE_PLAN',
                'template_text' => "You are planning a Shopify theme homepage. Choose sections that create a conversion-focused homepage with clear visual hierarchy. Use the brand's tone_of_voice and audience from the brand context. Order sections for the best visual flow and conversion (e.g. hero first, then value props, then content, then CTA). Use ONLY the provided section handles. If the brand has a logo (logo_path) or logo_design_notes, align section layout and styling with the logo design and use the logo where appropriate.\n\nCatalog summary:\n{{ catalog_summary }}\n\nAvailable section handles:\n{{ section_handles }}\n\nBrand context:\n{{ brand }}\n\nAdditional instructions from the user: {{ creative_brief }}\n\nOutput valid JSON with a key 'sections' (array of section handle strings in order).",
